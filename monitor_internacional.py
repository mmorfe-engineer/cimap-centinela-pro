"""
CENTINELA PRO — Monitor Internacional (Módulo B)
================================================

Orquesta el pipeline para el módulo INTERNACIONAL:
- 8 llamadas a Perplexity Sonar por región
- 2000 tokens Mistral
- Scope: EE.UU., Europa, AL, China, Rusia, Mercados/Energía/Sanciones
- Fuentes por región forzadas en prompts (BUG 6)
- Pages: /internacional/<correlativo>.html
- Teaser Telegram: prefijo 🌎 INTERNACIONAL

Diferencias vs monitor.py:
- Usa buscador.py con perfil "internacional" (8 llamadas)
- Prompts especializados por región con dominios forzados
- Parametros de entrega: prefijo_telegram="🌎 INTERNACIONAL", pages_subdir="internacional"
- Redactor recibe modulo="internacional" para paths
"""
...
def ejecutar_orquestacion_internacional(horas_atras: int | None) -> dict[str, Any]:
    """
    Orquesta el pipeline del módulo INTERNACIONAL.
    """
    # Configuración específica del módulo INTERNACIONAL
    os.environ["CENTINELA_PROFILE"] = "internacional"
    os.environ["MODULO"] = "internacional"
    ...
    resultado_entrega = entregar_informe(
        resultado_redaccion,
        correlativo,
        prefijo_telegram="🌎 INTERNACIONAL",
        pages_subdir="internacional"
    )
...
def _persist_outputs(salida: dict[str, Any]) -> dict[str, str]:
    ...
    latest = out_dir / "ultimo_resultado_internacional.json"
    ...
    snap = out_dir / f"{stamp}_internacional_{correlativo}.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="CENTINELA PRO - Monitor Internacional (Módulo B)"
    )
    parser.add_argument(
        "horas_atras",
        nargs="?",
    ...
    salida = ejecutar_orquestacion_internacional(horas_atras=horas_atras)
    paths = _persist_outputs(salida)

    meta = salida.get("metadata") or {}
    estado = salida.get("estado") or {}
    print(
        f"[centinela-internacional] correlativo={meta.get('correlativo')} "
        f"modulo={meta.get('modulo')} "
        f"horas_atras={horas_atras!r}"
    )
    print(
        f"[centinela-internacional] estado: habilitado={meta.get('estado_habilitado')} "
    ...
    print(f"[centinela-internacional] outputs: latest={paths['latest']} snapshot={paths['snapshot']}")
